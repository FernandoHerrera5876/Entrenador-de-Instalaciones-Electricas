...
public class SpawnPerforacion : MonoBehaviour
{
    ...
    public float tiempoDesactivacionAnimador = 10f; // Tiempo después del cual se desactiva el Animator
    public SpawnPanel spawnPanelScript; // Referencia al script SpawnPanel
    ...
    {
        // Verifica si el collider tiene el tag "Broca2" y si se puede instanciar
        if (other.CompareTag("Broca2") && puedeInstanciar)
        {
            ...
            if (puntoDeAparicion != null && objetoAparecer != null)
            {
                ...
                // Activa el Animator del objeto instanciado y programa su desactivación
                Animator animador = objetoInstanciado.GetComponent<Animator>();
                if (animador != null)
                {
                    animador.enabled = true;
                    StartCoroutine(DesactivarAnimatorDespuesDeTiempo(animador, tiempoDesactivacionAnimador));
                }
                ...
                // Obtiene la referencia al objeto instanciado por el script SpawnLiza y lo destruye
                GameObject objetoLizaInstanciado = spawnPanelScript.ObtenerObjetoInstanciado();
                if (objetoLizaInstanciado != null)
                {
                    Destroy(objetoLizaInstanciado);
                    Debug.Log("Objeto " + objetoLizaInstanciado.name + " destruido.");
                }
                else
                {
                    Debug.LogError("No se encontró un objeto instanciado por el script SpawnLiza en " + gameObject.name);
                }
            }
            else
            {
    ...
    {
        // Verifica si el objeto que salió del trigger tiene el tag "Broca2"
        if (other.CompareTag("Broca2"))
        {
    ...
    // Coroutine para desactivar el Animator después de un tiempo
    private IEnumerator DesactivarAnimatorDespuesDeTiempo(Animator animador, float tiempo)
    {
        yield return new WaitForSeconds(tiempo);
        if (animador != null)
        {
            animador.enabled = false;
            Debug.Log("Animator desactivado después de " + tiempo + " segundos.");
        }
    }
